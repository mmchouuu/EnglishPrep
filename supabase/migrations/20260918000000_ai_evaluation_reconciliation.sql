-- ============================================================================
-- Migration: Idempotent AI Evaluation Reconciliation & Storage Policies
-- File: supabase/migrations/20260918000000_ai_evaluation_reconciliation.sql
-- Scope: Versioned evaluations, partial unique active index, job claiming RPCs, Storage RLS, and Dashboard RPC
-- ============================================================================
...
BEGIN
  IF EXISTS (
    ...
    -- Check if PK constraint is on (response_id) alone
    IF EXISTS (
      SELECT 1
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE table_name = 'practice_response_evaluations' 
...
-- Self-referential FK for evaluation version history
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
...
-- Partial Unique Index for Active Job Idempotency
DROP INDEX IF EXISTS public.idx_active_evaluation_per_response;
...
-- ----------------------------------------------------------------------------
-- 2. JOB CLAIMING & RECOVERY RPC FUNCTIONS (FOR C# BACKGROUND WORKER SPEC)
-- ----------------------------------------------------------------------------
...
-- Speaking Images: Public Read, Service-Role Write
INSERT INTO storage.buckets (id, name, public, avif_autodetection, file_size_limit, allowed_mime_types)
VALUES (
  'speaking-images',
  'speaking-images',
  true, -- PUBLIC BUCKET FOR QUESTION IMAGES
  false,
  10485760, -- 10MB
...
DROP POLICY IF EXISTS "Admin Upload Access for Speaking Images" ON storage.objects;
CREATE POLICY "Admin Upload Access for Speaking Images"
  ON storage.objects FOR INSERT TO service_role
  WITH CHECK (bucket_id = 'speaking-images');

DROP POLICY IF EXISTS "Admin Update Access for Speaking Images" ON storage.objects;
CREATE POLICY "Admin Update Access for Speaking Images"
  ON storage.objects FOR UPDATE TO service_role
  USING (bucket_id = 'speaking-images')
  WITH CHECK (bucket_id = 'speaking-images');

DROP POLICY IF EXISTS "Admin Delete Access for Speaking Images" ON storage.objects;
CREATE POLICY "Admin Delete Access for Speaking Images"
  ON storage.objects FOR DELETE TO service_role
  USING (bucket_id = 'speaking-images');

-- ----------------------------------------------------------------------------
-- 4. REFINED DASHBOARD STATS RPC FUNCTION
-- ----------------------------------------------------------------------------
...
BEGIN
  ...
  SELECT a.id AS attempt_id, e.status, e.normalized_score, e.cefr_level, e.completed_at
  INTO v_rec
  FROM public.practice_attempts a
  ...
  IF FOUND THEN
    ...
    IF v_rec.status = 'completed' AND v_rec.normalized_score IS NOT NULL THEN
      v_w_score := LEAST(100, GREATEST(0, ROUND(v_rec.normalized_score)));
      v_w_cefr := v_rec.cefr_level;
    ELSE
      v_w_score := NULL;
      v_w_cefr := NULL;
    END IF;
  ...
  RETURN jsonb_build_object(
    'overall', jsonb_build_object(
      ...
      'latestSkill', CASE WHEN v_w_score IS NOT NULL THEN 'writing' WHEN v_s_score IS NOT NULL THEN 'speaking' WHEN v_r_score IS NOT NULL THEN 'reading' ELSE 'listening' END,
      'cefrLevel', COALESCE(v_w_cefr, v_s_cefr),
