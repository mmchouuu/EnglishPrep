      ...
      {setsToRender.map((currentSet, sIdx) => {
        ...
        return (
          ...
          >
            <TopicHeaderBanner
              topicName={topicName}
              setInfo={`Set ${actualSetIdx + 1} of ${allSets.length}`}
            ...
            >
              ...
              <div className="space-y-6 pt-2">
                {(currentSet.questions || []).map((q, qIdx) => {
                  const qKey = q.id || q.sourceKey || `${currentSet.id}_p4_q${qIdx + 1}`;
                  const selectedOpt = userAnswers[qKey];
                  const isSub = submittedQuestions[q.id || qKey];
                  const isFlagged = markedQuestions[q.id || qKey];

                  return (
                    <div
                      key={qKey}
                      id={`question-${q.id || qKey}`}
                    ...
                    >
                      ...
                      <div className="flex items-start justify-between gap-4 flex-wrap pb-1">
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          <button
                            onClick={() => onToggleMark(q.id || qKey)}
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 pt-1">
                        {(q.options || []).map((opt, idx) => {
                          ...
                          const setSubbed = submittedQuestions[currentSet.id];
                          const itemSubbed = isSub || setSubbed;
                          ...
                          if (itemSubbed && q.correctAnswer !== undefined && q.correctAnswer !== null) {
                            const correctKey = typeof q.correctAnswer === 'number'
                          ...
                          return (
                            <button
                              ...
                              className="flex items-center gap-2.5 px-3.5 py-2.5 rounded-xl border text-left text-xs sm:text-sm transition-all focus:outline-none focus:ring-2 focus:ring-blue-500"
                              style={{
                      ...
                      {(isSub || submittedQuestions[currentSet.id]) && (
                        (() => {
                          const correctKey = q.correctAnswer !== undefined && q.correctAnswer !== null
                          ...
                          return (
                            <div className={`p-3.5 rounded-xl border text-xs leading-relaxed flex items-start gap-2.5 ${
                              ...
                              <div>
                                ...
                                {q.explanation || (correctKey ? `Correct answer is Option ${correctKey}.` : 'Answer checked.')}
                              </div>
                            </div>
                          );
                        })()
              ...
              <div className="pt-5 border-t border-slate-200/80 dark:border-slate-800 flex items-center justify-end gap-3 flex-wrap">
                <div className="flex items-center gap-3">
                  ...
                  <button
                    onClick={() => {
                      if (onSubmitSingle) {
                        onSubmitSingle(currentSet.id);
                        (currentSet.questions || []).forEach(q => {
                          const qKey = q.id || q.sourceKey;
                          onSubmitSingle(qKey);
                        });
                      }
                    }}
                    disabled={submittedQuestions[currentSet.id]}
                    className={`flex items-center gap-1.5 px-5 py-2 rounded-xl text-xs font-bold transition-all border border-blue-500 text-[#2563eb] hover:bg-blue-50 ${
                      submittedQuestions[currentSet.id] ? 'opacity-50 cursor-not-allowed' : ''
                    }`}
                  >
                    <Check className="w-4 h-4" />
                    <span>{submittedQuestions[currentSet.id] ? 'Set Checked' : 'Check this set'}</span>
                  </button>
                </div>
              </div>
